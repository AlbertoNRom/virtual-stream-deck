...
async function uploadSharedSound(soundId: string, localPath: string): Promise<string> {
  try {
    const fileBuffer = fs.readFileSync(localPath);
    
    //@ts-ignore
    const file = new File([fileBuffer], `${soundId}.mp3`, { type: 'audio/mpeg' });
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<{message: string, sounds?: number, keys?: number, error?: string, details?: string}>) {
  ...
  try {
    ...
    const glassBreakUrl = await uploadSharedSound(soundIds.glassBreak, path.join(soundExamplesPath, 'glass-break.mp3'));
    const cinematicHitUrl = await uploadSharedSound(soundIds.cinematicHit, path.join(soundExamplesPath, 'cinematic-hit-3.mp3'));
    const policeSirenUrl = await uploadSharedSound(soundIds.policeSiren, path.join(soundExamplesPath, 'police-sirens.mp3'));
    ...
    if (soundsToInsert.length > 0) {
      const { data, error: soundError } = await supabase
        .from('sounds')
    ...
    return res.status(200).json({ 
      message: 'Stream deck successfully configured: 3 sounds and 3 keys created',
    });
