...
export const useStreamDeckHotkeys = () => {
	...
	const validHotkeys = useMemo(() => {
		...
		setRegisteredHotkeys(
			processed.map((p) => ({ hotkey: p.normalized, soundId: p.soundId })),
		);
	...
	useHotkeys(
		...
		(event, handler) => {
			...
			const mainKey = handler.keys
				? handler.keys[handler.keys.length - 1].toLowerCase()
				: '';
			...
			// Find match
			const match = validHotkeys.find(
				(item) => item.normalized === pressedHotkey,
			);
