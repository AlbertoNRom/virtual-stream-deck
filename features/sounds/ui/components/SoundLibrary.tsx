...
export const SoundLibrary = ({
	...
	// Hook BLoC
	const { upload, remove, ensureKeyForSound, isUploading, isRemoving } =
		useSoundLibrary();
			...
			<CardContent className="space-y-3 sm:space-y-4 p-4 sm:p-6">
				...
				{sounds.length >= 7 && sounds.length < 9 && (
					<div className="p-2 sm:p-3 mb-2 sm:mb-3 bg-yellow-500/10 border border-yellow-500/30 rounded-lg text-xs sm:text-sm">
						<p className="font-medium text-yellow-500">
							Warning! You are close to the sound limit
						</p>
						<p className="text-xs text-muted-foreground mt-1">
							You have {sounds.length} of 9 allowed sounds.
						</p>
					</div>
				)}
				...
				{sounds.length >= 9 && (
					<div className="p-2 sm:p-3 mb-2 sm:mb-3 bg-red-500/10 border border-red-500/30 rounded-lg text-xs sm:text-sm">
						<p className="font-medium text-red-500">
							You have reached the sound limit
						</p>
						<p className="text-xs text-muted-foreground mt-1">
							Delete some sounds to add more.
						</p>
					</div>
				)}
				...
				<div className="mt-5 sm:mt-6 space-y-2 sm:space-y-3">
					{filteredSounds.map((sound) => (
						...
						>
							<span
								className="truncate flex-1 text-sm sm:text-base max-w-[150px] sm:max-w-[200px] block"
								title={sound.name}
							>
