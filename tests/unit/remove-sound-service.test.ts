...
describe('RemoveSound Service', () => {
	...
	describe('execute', () => {
		it('should remove sound and associated keys successfully', async () => {
			...
			expect(mockKeyRepository.removeBySoundId).toHaveBeenCalledWith(
				'user-1',
				'sound-1',
			);
			expect(mockSoundRepository.remove).toHaveBeenCalledWith(
				'sound-1',
				'user-1',
			);
		...
		it('should handle storage removal failure gracefully', async () => {
			...
			vi.mocked(mockSoundRepository.findById).mockResolvedValue(sound);
			vi.mocked(mockStorage.removeByPublicUrl).mockRejectedValue(
