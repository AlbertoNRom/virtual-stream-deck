		...
		<div data-testid="sound-library">
			...
			{sounds.map((sound) => (
				<div key={sound.id} data-testid={`sound-${sound.id}`}>
					...
					<button type="button" onClick={() => playSound(sound.id)}>
						Play
					</button>
	...
	return (
		<div data-testid="stream-deck-grid">
			...
			<div data-testid="grid-config">
				{gridConfig.rows}x{gridConfig.columns}
			</div>
			<div className="grid">
				{Array.from(
					{ length: gridConfig.rows * gridConfig.columns },
					(_, index) => {
	...
	beforeEach(() => {
		mockStoreState = {
			...
			removeSound: vi.fn((id) => {
				mockStoreState.sounds = mockStoreState.sounds.filter(
					(s: SoundRow) => s.id !== id,
				);
			}),
			setStreamDeckKeys: vi.fn(),
			updateKey: vi.fn((key: StreamDeckKeyRow) => {
				const keyIndex = mockStoreState.streamDeckKeys.findIndex(
					(k: StreamDeckKeyRow) => k.id === key.id,
				);
				if (keyIndex !== -1) {
